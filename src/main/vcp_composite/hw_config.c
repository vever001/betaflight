 ...
 * Return         : None.
 *******************************************************************************/
uint32_t Joystick_Send(uint8_t *ptrBuffer, uint32_t sendLength)
{
    // Try to load some bytes if we can
    if (sendLength) {
        UserToPMABufferCopy(ptrBuffer, ENDP3_TXADDR, sendLength);
        //or USB_SIL_Write(EP3_IN, data, len);
        SetEPTxCount(ENDP3, sendLength);
        SetEPTxValid(ENDP3);
    }
